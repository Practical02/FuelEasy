...
vi.mock('./storage', async () => {
  const actual = await import('./storage');
  return {
    ...actual,
    storage: {
      ...actual.storage,
...
describe('API Routes', () => {
  ...
  describe('/api/account-heads', () => {
    it('GET /api/account-heads should return a list of account heads', async () => {
      ...
      (storage.getAccountHeads as unknown as import("vitest").MockInstance).mockResolvedValue(mockAccountHeads);
    ...
    it('POST /api/account-heads should create a new account head', async () => {
      ...
      (storage.createAccountHead as unknown as import("vitest").MockInstance).mockResolvedValue(createdAccountHead);
  ...
  describe('/api/payments', () => {
    it('POST /api/payments should create a payment and trigger cashbook entry', async () => {
      ...
      (storage.createPayment as unknown as import("vitest").MockInstance).mockResolvedValue(createdPayment);
      (storage.getSale as unknown as import("vitest").MockInstance).mockResolvedValue({ id: 'sale-123', totalAmount: '200.00', saleStatus: 'Invoiced' });
      (storage.getPaymentsBySale as unknown as import("vitest").MockInstance).mockResolvedValue([]);
      (storage.updateSaleStatus as unknown as import("vitest").MockInstance).mockResolvedValue({});
      const createdEntry = { id: 'entry-id', ...mockPaymentData };
      (storage.createCashbookEntry as unknown as import("vitest").MockInstance).mockResolvedValue(createdEntry);
  ...
  describe('/api/stock', () => {
    it('POST /api/stock should create stock and trigger cashbook entry', async () => {
      ...
      const updatedDebt = { id: 'debt-id', isPending: 0 };
      (storage.markDebtAsPaid as unknown as import("vitest").MockInstance).mockResolvedValue(updatedDebt);
      (storage.createStock as unknown as import("vitest").MockInstance).mockResolvedValue(createdStock);
  ...
  describe('/api/cashbook', () => {
    it('GET /api/cashbook should return cashbook entries', async () => {
      ...
      (storage.getCashbookEntries as unknown as import("vitest").MockInstance).mockResolvedValue(mockCashbookEntries);
